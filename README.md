...
Package for clean, insightful dashboards targeting competitive MTG Arena analysis.
Covers 2 of 3 layers in the `MTGA_DATA` environment: **data analysis** and **visualisation**.
...
<p align="center">
  ...
  <img src="images/07_time_series.png" width="48%" />
</p>

## Usage

1. Install package (`pip install -e .` from repo root) — requires Manim + pandas dependencies pre-installed.
2. Feed `analysis/` module raw event data (from `mtga_scraper` or `mtga_data_bot` outputs) to generate metagame stats.
3. Select modality — general functions run regardless; add Tournament/League or Metagame Challenge functions as needed.
4. Pass analysis DataFrame(s) into matching `visualisation/` dashboard function to render Manim scene.
5. Export rendered PNG/video via Manim CLI; place in `images/` for README/dashboard embedding.
