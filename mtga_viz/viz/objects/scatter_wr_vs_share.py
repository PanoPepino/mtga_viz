    ...
    def __init__(
        ...
        **kwargs,
        ...
        axis_labels = self.plane.get_axis_labels(
            x_label=MathTex("Presence\\,(\\%)", color=WHITE, font_size=20),
            y_label=MathTex("WR\\,(\\%)", color=WHITE, font_size=20),
        )
        axis_labels[0].next_to(self.plane.x_axis.get_end(), DOWN, aligned_edge=RIGHT, buff=buff)
        axis_labels[1].next_to(self.plane.y_axis.get_end(), UP, aligned_edge=LEFT, buff=buff)

        dots = Group()
        for lab in labels:
            tag = Pin(
                label=lab,
                stroke_width=2*stroke_w,
        ...
        self.add(self.plane, axis_labels, self.frame, dots)
